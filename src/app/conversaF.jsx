...
import { addMensagem, getMensagens } from "@/database/database";
        ...
        <View style={[styles.proposalBubble, isMe && styles.myProposalBubble]}>
            ...
            <View style={styles.proposalValueRow}>
                ...
                <View style={styles.proposalValueContainer}>
                    <Ionicons name="cash-outline" size={20} color={isMe ? colors.creme : colors.marrom} />
                    <Text style={[styles.proposalValue, isMe && styles.myProposalValue]}>R${proposal.value},00</Text>
                </View>
            </View>
            <Text style={[styles.proposalDescription, isMe && styles.myProposalText]}>{proposal.description}</Text>
...
export default function ChatFreelancer() {
    ...
    const handleConfirmProposal = async () => {
        ...
        const time = getCurrentTime();
        const id = await addMensagem(userId, contactId, userType, true, 'proposal', {
            propostaValor: value,
            propostaDescricao: proposalDescription,
        }, time);
        const newProposal = {
            id,
            type: "proposal",
            proposal: { value, description: proposalDescription, time, status: 'pending' },
            isMe: true,
            time,
        };
        ...
        <View style={{ flex: 1, backgroundColor: colors.creme }}>
            ...
            >
                ...
                >
                    ...
                    {messages.map((msg) => (
                        msg.type === "proposal" && msg.proposal ? (
                            <FreelancerProposalMessage key={msg.id} proposal={msg.proposal} isMe={msg.isMe} />
                        ) : (
                ...
                <View style={styles.inputArea}>
                    ...
                    <TouchableOpacity style={styles.proposalButton} onPress={() => setShowProposalModal(true)}>
                ...
                {/* Modal proposta */}
                <Modal visible={showProposalModal} transparent animationType="fade" onRequestClose={() => setShowProposalModal(false)}>
                        ...
                        <View style={styles.proposalModal}>
                            ...
                            {proposalValue ? (
                                <View style={styles.discountContainer}>
                                    <Text style={styles.discountText}>Valor total a receber:</Text>
                                    ...
                                    <View style={styles.discountedValueContainer}>
                                        ...
                                        <Text style={styles.discountedValue}>R${calculateDiscountedValue()}</Text>
                                    </View>
                                </View>
                            ) : null}
                            <View style={styles.modalActions}>
                                <TouchableOpacity style={styles.cancelButton} onPress={() => { setShowProposalModal(false); setProposalValue(""); setProposalDescription(""); }}>
                                    <Text style={styles.cancelButtonText}>Cancelar</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={styles.confirmButton} onPress={handleSendProposal}>
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={styles.confirmationModal}>
                            ...
                            <View style={styles.confirmationValueContainer}>
                                <Ionicons name="cash-outline" size={24} color={colors.marrom} />
                                <Text style={styles.confirmationValue}>R${proposalValue},00</Text>
                            </View>
                            ...
                            <View style={styles.modalActions}>
                                ...
                                <TouchableOpacity style={styles.confirmButton} onPress={handleConfirmProposal}>
...
const styles = StyleSheet.create({
    ...
    myProposalBubble: { backgroundColor: colors.marromClaro, borderColor: colors.marrom },
    proposalTitle: { fontSize: 14, fontFamily: 'KohoBold', color: colors.marrom, marginBottom: 10 },
    myProposalTitle: { color: colors.creme },
    ...
    myProposalValue: { color: colors.creme },
    myProposalText: { color: colors.creme },
    proposalDescription: { fontSize: 13, fontFamily: 'KohoLight', color: colors.cinza, marginBottom: 15, lineHeight: 18 },
    proposalStatus: { backgroundColor: colors.dourado, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 5, alignSelf: 'flex-start', marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 6 },
    proposalStatusAccepted: { backgroundColor: colors.verde },
    proposalStatusText: { color: colors.creme, fontSize: 12, fontFamily: 'KohoMedium' },
    ...
    proposalButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: colors.marrom, justifyContent: 'center', alignItems: 'center' },
    ...
    proposalModal: { width: '85%', backgroundColor: colors.creme, borderRadius: 15, padding: 25 },
    proposalModalTitle: { fontSize: 18, fontFamily: 'KohoBold', color: colors.marrom, textAlign: 'center', marginTop: 8 },
    proposalModalSubtitle: { fontSize: 14, fontFamily: 'KohoRegular', color: colors.cinza, textAlign: 'center', marginBottom: 20 },
    valueInputContainer: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: colors.marromClaro, borderRadius: 8, paddingHorizontal: 15, marginBottom: 15, gap: 10 },
    valueInput: { flex: 1, fontSize: 18, fontFamily: 'KohoBold', color: colors.preto, paddingVertical: 12 },
    descriptionInput: { borderWidth: 1, borderColor: colors.marromClaro, borderRadius: 8, padding: 15, fontSize: 14, fontFamily: 'KohoRegular', color: colors.preto, textAlignVertical: 'top', marginBottom: 20, minHeight: 80 },
    discountContainer: { backgroundColor: colors.bege, padding: 15, borderRadius: 8, marginBottom: 20, alignItems: 'center' },
    discountText: { fontSize: 14, fontFamily: 'KohoRegular', color: colors.cinza, marginBottom: 10 },
    discountRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 10 },
    originalValue: { fontSize: 16, fontFamily: 'KohoRegular', color: colors.cinza, textDecorationLine: 'line-through' },
    discountPercentage: { fontSize: 16, fontFamily: 'KohoBold', color: colors.marrom },
    discountedValueContainer: { flexDirection: 'row', alignItems: 'center', gap: 8 },
    discountedValue: { fontSize: 24, fontFamily: 'KohoBold', color: colors.marrom },
    confirmationModal: { width: '85%', backgroundColor: colors.creme, borderRadius: 15, padding: 25 },
    confirmationTitle: { fontSize: 18, fontFamily: 'KohoBold', color: colors.marrom, textAlign: 'center', marginTop: 8, marginBottom: 15 },
    confirmationValueContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 15 },
    confirmationValue: { fontSize: 24, fontFamily: 'KohoBold', color: colors.marrom },
    confirmationText: { fontSize: 14, fontFamily: 'KohoRegular', color: colors.cinza, textAlign: 'center', marginBottom: 25, lineHeight: 20 },
    modalActions: { flexDirection: 'row', gap: 12 },
    cancelButton: { flex: 1, padding: 14, borderRadius: 8, borderWidth: 1, borderColor: colors.marromClaro, alignItems: 'center' },
    cancelButtonText: { color: colors.cinza, fontSize: 14, fontFamily: 'KohoMedium' },
    confirmButton: { flex: 1, backgroundColor: colors.marrom, padding: 14, borderRadius: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 },
    confirmButtonText: { color: colors.creme, fontSize: 14, fontFamily: 'KohoBold' },
});
